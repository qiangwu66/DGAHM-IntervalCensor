...
#%% --------------------------
def LFD_gamma(Zi_train,Z_train,X_train,U_train,V_train,De1_train,De2_train,De3_train,g_train,theta,C,m,nodevec,n_layer,n_node,n_lr,n_epoch):
    ...
    def Loss(De1,De2,De3,Zi,Z,U,V,g_X,theta,C,m,nodevec,a_b_c):
        ...
        L_gamma = De1 * Zi * f_U * torch.exp(-f_U) / (1 - torch.exp(-f_U) + 1e-4) + De2 * Zi * (f_V * torch.exp(-f_V)- f_U * torch.exp(-f_U)) / (torch.exp(-f_U) - torch.exp(-f_V) + 1e-4) - De3 * Zi * f_V
