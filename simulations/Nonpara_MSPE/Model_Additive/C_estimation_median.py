...
def C_est_median(m, U, V, De1, De2, De3, g_X, nodevec, C0):
    ...
    Iu_2 = I_U(m, np.clip(U * np.exp(g_X[:,0]) / 2, 0, 70), nodevec)
    Iuv_2 = I_U(m, np.clip((U + V) * np.exp(g_X[:,0]) / 2, 0, 70), nodevec)
    B_U_2 = B_S2(m, np.clip(U * np.exp(g_X[:,0]) / 2, 0, 70), nodevec)
    B_U_V_2 = B_S2(m, np.clip((U + V) * np.exp(g_X[:,0]) / 2, 0, 70), nodevec)
