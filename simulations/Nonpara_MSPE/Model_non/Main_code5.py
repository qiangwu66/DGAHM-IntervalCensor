...
test_data = generate_case_5(500, corr, tau)
X_test = test_data['X']
...
def one_replication(b,
    ...
    train_data = generate_case_5(n, corr, tau)
    # valid data for early stopping/model selection
    valid_data = generate_case_5(valid_n, corr, tau)
...
for i in range(len(Set_n)):
    ...
    results = Parallel(n_jobs=15, backend="loky", verbose=0)(
...
result_MSPE.to_csv('MSPE_Case5.csv', index=False)
