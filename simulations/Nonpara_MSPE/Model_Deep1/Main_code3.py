...
test_data = generate_case_3(500, corr, tau)
X_test = test_data['X']
...
nodevec = np.array(np.linspace(0, 80, m + 2), dtype="float32")
...
def one_replication(b,
    ...
    train_data = generate_case_3(n, corr, tau)
    # valid data for early stopping/model selection
    valid_data = generate_case_3(valid_n, corr, tau)
...
result_MSPE.to_csv('MSPE_Case3.csv', index=False)
