...
def C_est_median(m, U, V, De1, De2, De3, g_X, nodevec, C0):
    Iv = I_U(m, np.clip(V * np.exp(g_X[:,0]), 0, 80), nodevec)
    Iu_2 = I_U(m, np.clip(U * np.exp(g_X[:,0]) / 2, 0, 80), nodevec)
    Iuv_2 = I_U(m, np.clip((U + V) * np.exp(g_X[:,0]) / 2, 0, 80), nodevec)
    B_U_2 = B_S2(m, np.clip(U * np.exp(g_X[:,0]) / 2, 0, 80), nodevec)
    B_U_V_2 = B_S2(m, np.clip((U + V) * np.exp(g_X[:,0]) / 2, 0, 80), nodevec)
