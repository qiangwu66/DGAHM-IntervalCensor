...
def g_L(train_data,X_test,X_subject,C,m,nodevec):
    ...
    def GL(*args):
        ...
        Iu = I_U(m, np.clip(U_train * np.exp(Z_train * b[2*d+2] + g1_X), 0, 95), nodevec)
        Iv = I_U(m, np.clip(V_train * np.exp(Z_train * b[2*d+2] + g1_X), 0, 95), nodevec)
        Ezg = np.exp(Z_train * b[2*d+3] + g2_X)
    ...
    init_b = np.zeros(2*d+4)
    init_b[2*d+2] = 1.0
    init_b[2*d+3] = 1.0
    
    eps = 1e-5
    bnds = [(None, None)] * (2*d + 2) + [(eps, 20 - eps), (eps, 20 - eps)]
    
    g_linear_para = spo.minimize(GL, init_b, method='SLSQP', bounds=bnds)['x']
    ...
    theta = g_linear_para[(2*d+2):(2*d+4)]
