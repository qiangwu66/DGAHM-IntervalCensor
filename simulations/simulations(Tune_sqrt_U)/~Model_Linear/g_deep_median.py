...
#%% --------------------------
def g_D_median(train_data, valid_data, X_test, X_subject, theta, C, m,nodevec, n_layer, n_node, n_lr, n_epoch):
    ...
    def my_loss(De1, De2, De3, Z, theta, C, m, U, V, nodevec, g_X):
        ...
        scale = torch.exp(Z * theta[0] + g1) 
        Ezg  = torch.exp(Z * theta[1] + g2)  
        ...
        term1 = torch.log(torch.clamp(c1, min=eps)) + Z * (theta[0] + theta[1]) + g1 + g2 - d1

        term2 = torch.log(torch.clamp(c2, min=eps)) + Z * (theta[0] + theta[1]) + g1 + g2 - d2
    ...
    g_test = g_test.detach().cpu().numpy()
    g_subject = g_subject.detach().cpu().numpy()
