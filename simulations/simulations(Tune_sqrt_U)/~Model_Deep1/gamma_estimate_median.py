...
def gamma_est(beta, De1, De2, De3, Z, U, V, C, m, g_X, nodevec):
    def BF(*args):
        b = args[0]
        Iv = I_U(m, np.clip(V * np.exp(Z * beta + g_X[:,0]), 0, 100), nodevec)
        Iu_2 = I_U(m, np.clip(U * np.exp(Z * beta + g_X[:,0]) / 2, 0, 100), nodevec)
        Iuv_2 = I_U(m, np.clip((U + V) * np.exp(Z * beta + g_X[:,0]) / 2, 0, 100), nodevec)
        B_U_2 = B_S2(m, np.clip(U * np.exp(Z * beta + g_X[:,0]) / 2, 0, 100), nodevec)
        B_U_V_2 = B_S2(m, np.clip((U + V) * np.exp(Z * beta + g_X[:,0]) / 2, 0, 100), nodevec)
